...
export class IntegrationGateService {
  ...
  validateIntegrationEvidence(
    ...
    return {
      ...
      notes: validated
        ? "Evidence meets integration criteria"
        : "Evidence needs more grounding in daily reality",
    };
  }
  ...
  private getValidationCriteria(evidenceType: string): string[] {
    ...
    return (
      criteria[evidenceType as keyof typeof criteria] || criteria.daily_practice
    );
  }
  ...
  private assessEvidence(
    ...
    const evidenceText =
      `${evidence.description} ${evidence.realWorldContext}`.toLowerCase();
    ...
    const groundingScore = groundingWords.filter((word) =>
      evidenceText.includes(word),
    ).length;
    const bypassingScore = bypassingWords.filter((word) =>
      evidenceText.includes(word),
    ).length;
  ...
  handleBypassAttempt(reflectionGap: ReflectionGap, userId: string): string {
    ...
    return (
      messages[reflectionGap.bypassAttempts as keyof typeof messages] ||
      messages[4]
    );
  }
  ...
  private triggerProfessionalSupportReview(userId: string): void {
    ...
    console.log(
      `Professional support review triggered for user ${userId} due to consistent integration avoidance`,
    );
  }
  ...
  private assessEvidenceQuality(evidence: IntegrationEvidence[]): number {
    ...
    const qualityScores = evidence.map((e) => {
      ...
      const daysSince = Math.floor(
        (Date.now() - e.date.getTime()) / (1000 * 60 * 60 * 24),
      );
  ...
  private calculateCurrentSpiralDepth(
