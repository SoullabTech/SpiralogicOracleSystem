...
export class JungBuddhaDemo {
  ...
  async demonstrateResponses() {
    ...
    const angerHybrid = await this.simulateResponse(
      "This anger keeps coming back",
    );
    ...
    const fearJung = await this.simulateResponse(
      "I hate being so afraid all the time",
    );
    console.log("🧿 Jung Response:", fearJung.response);
    console.log("");

    console.log('💭 User: "I am just a fearful person"');
    const fearBuddha = await this.simulateResponse(
    ...
    console.log(
      '💭 User: "This fear keeps returning when I try to take risks"',
    );
    const fearHybrid = await this.simulateResponse(
      "This fear keeps returning when I try to take risks",
    );
    ...
    const shameJung = await this.simulateResponse(
    ...
    const shameBuddha = await this.simulateResponse(
    ...
    const sadnessHybrid = await this.simulateResponse(
    ...
    console.log(
      '💭 User: "I keep getting stuck in the same relationship patterns"',
    );
    const relationshipPattern = await this.simulateResponse(
      "I keep getting stuck in the same relationship patterns",
    );
    ...
    console.log(
      '💭 User: "This is just who I am - someone who fails at everything"',
    );
    const identityStory = await this.simulateResponse(
      "This is just who I am - someone who fails at everything",
    );
...
export const JungBuddhaResponseExamples = {
  ...
  shame: {
    ...
    identification: {
      ...
      expected: "buddha",
      response:
        "You say you are broken, but what were you before this story began? Who is the one who knows about being broken?",
    },
  },
  ...
  patterns: {
    cyclical: {
      ...
      expected: "hybrid",
      response:
        "This pattern of relationship patterns is both teacher and test. What wants to be integrated? What wants to be released?",
    },
    identity_story: {
...
export async function testJungBuddhaRouting() {
  ...
  // Test each category
  for (const [emotion, patterns] of Object.entries(
