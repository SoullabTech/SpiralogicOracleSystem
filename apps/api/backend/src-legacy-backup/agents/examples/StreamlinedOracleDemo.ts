...
export class StreamlinedOracleDemo {
  ...
  async demonstrateAllModes() {
    ...
    const testInputs = [
      {
        ...
        description:
          "Shadow work needed - should trigger Alchemist mode suggestion",
      },
      {
    ...
    for (const mode of ["alchemist", "buddha", "sage", "mystic", "guardian"]) {
      ...
      if (status.wisdomRouting) {
        console.log(
          `   Routing: ${status.wisdomRouting.approach} (${(status.wisdomRouting.confidence * 100).toFixed(0)}%)`,
        );
  ...
  async demonstrateModeSuggestions() {
    ...
    for (const testCase of testCases) {
      ...
      console.log(
        `🎯 Expected: ${testCase.expectedSuggestion} (${testCase.reason})`,
      );

      const suggestion = await this.oracle.suggestModeForInput(testCase.input);

      console.log(
        `🤖 Suggested: ${suggestion.suggestedMode || "No change needed"}`,
      );
      if (suggestion.suggestedMode) {
        ...
        const correct =
          suggestion.suggestedMode === testCase.expectedSuggestion;
