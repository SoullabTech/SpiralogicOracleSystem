...
export class ExampleOracle {
  ...
  async respondToUser(userInput: string, userHistory: any): Promise<string> {
    ...
    console.log(
      `🎯 Used ${routing.approach} approach (confidence: ${routing.confidence})`,
    );
  ...
  private buildUserContext(input: string, history: any): UserContext {
    ...
    if (
      input.toLowerCase().includes("need to") ||
      input.toLowerCase().includes("must")
    ) {
      patterns.push({
        type: "grasping",
        content: input,
    ...
    if (
      input.toLowerCase().includes("avoid") ||
      input.toLowerCase().includes("don't want")
    ) {
      patterns.push({
        type: "avoidance",
        content: input,
  ...
  private generateBuddhaResponse(
    input: string,
