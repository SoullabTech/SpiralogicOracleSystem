...
exports.safetyService = exports.SafetyModerationService = void 0;
...
class SafetyModerationService {
     ...
     */
    async moderateInput(input, userId) {
        try {
            ...
            if (crisisDetected)
                severity = 'critical';
            else if (result.flagged || emotionalState.needsSupport)
                severity = 'high';
            ...
            logger_1.logger.error('Moderation service error:', error);
    ...
    detectCrisisLanguage(input) {
        ...
        return this.crisisKeywords.some(keyword => lowerInput.includes(keyword.toLowerCase()));
    }
    /**
    ...
    async analyzeEmotionalState(input) {
        try {
            ...
            logger_1.logger.error('Emotional analysis error:', error);
            return this.analyzeEmotionalStateFallback(input);
        }
    }
    /**
     ...
     */
    analyzeEmotionalStateFallback(input) {
        const lowerInput = input.toLowerCase();
        // Crisis indicators
     ...
     */
    generateSupportiveResponse(emotionalState, crisisDetected) {
        if (crisisDetected) {
     ...
     */
    getSupportResources(severity, emotionalState) {
        const resources = [];
        if (severity === 'critical') {
            const crisisResources = this.resourceDatabase.get('crisis');
            if (crisisResources) {
                crisisResources.immediate.forEach((resource) => {
        ...
        if (emotionalState.needsSupport) {
            ...
            if (supportResources) {
                Object.values(supportResources).flat().forEach((resource) => {
     ...
     */
    async logSafetyMetrics(metrics) {
        try {
            ...
            logger_1.logger.error('Failed to log safety metrics:', error);
    ...
    async moderateResponse(response) {
        try {
            ...
            logger_1.logger.error('Response moderation error:', error);
     ...
     */
    checkSpiritualAppropriateness(content) {
        ...
        const foundPattern = inappropriatePatterns.find(pattern => lowerContent.includes(pattern));
        if (foundPattern) {
            return {
                safe: false,
     ...
     */
    generateCrisisResponse(emotionalState) {
...
exports.SafetyModerationService = SafetyModerationService;
// Export singleton instance
exports.safetyService = new SafetyModerationService();
