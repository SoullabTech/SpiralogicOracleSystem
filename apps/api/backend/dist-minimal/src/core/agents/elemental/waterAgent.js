...
const WaterIntelligence = {
    detectWaterType: (input, context) => {
        ...
        if (lowerInput.includes("overwhelm") ||
        ...
        if (lowerInput.includes("ashamed") ||
        ...
        if (lowerInput.includes("relationship") ||
    ...
    craftWaterResponse: (input, waterType, memories) => {
...
Your emotional waters hold the key to your authentic self - your inner gold. Let's listen to what they're trying to tell you.`;
    ...
    addWaterSignature: (response, waterType) => {
        const signatures = {
...
class WaterAgent extends ArchetypeAgent_1.ArchetypeAgent {
    constructor(oracleName = "Aquaria", voiceProfile, phase = "initiation") {
...
${modelResponse.response}`;
    ...
    assessEmotionalIntensity(input) {
        ...
        if (mediumIntensityWords.some((word) => lowerInput.includes(word)))
            return 0.6;
        if (lowIntensityWords.some((word) => lowerInput.includes(word)))
            return 0.3;
        return 0.5; // baseline emotional presence
    }
    assessHealingPotential(input) {
    ...
    extractWaterReflections(content) {
        const reflections = [];
