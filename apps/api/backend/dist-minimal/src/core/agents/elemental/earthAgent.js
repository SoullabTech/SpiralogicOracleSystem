...
const EarthIntelligence = {
    detectEarthType: (input, context) => {
        ...
        if (lowerInput.includes("burnout") ||
    ...
    craftEarthResponse: (input, earthType, memories) => {
...
What in your life feels most solid and trustworthy right now? Let's start there and grow from that foundation. What wants to be built from this place of strength?`;
    ...
    addEarthSignature: (response, earthType) => {
        const signatures = {
...
class EarthAgent extends ArchetypeAgent_1.ArchetypeAgent {
    constructor(oracleName = "Terra", voiceProfile, phase = "initiation") {
...
${modelResponse.response}`;
    ...
    assessManifestationReadiness(input) {
    ...
    extractEarthReflections(content) {
        const reflections = [];
