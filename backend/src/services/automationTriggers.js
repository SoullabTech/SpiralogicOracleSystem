...
exports.triggerUserSignupAutomation = exports.analyzeConversationForAutomation = exports.AutomationTriggers = void 0;
...
class AutomationTriggers {
    ...
    static async analyzeForBreakthrough(conversationData) {
        try {
            ...
            if (breakthroughAnalysis.isBreakthrough) {
                ...
                await soulMemoryService_1.SoulMemoryService.store({
                    userId,
     ...
     */
    static async analyzeForStuckPatterns(userId) {
        try {
            // Get recent user memories
            const recentMemories = await soulMemoryService_1.SoulMemoryService.retrieve({
                userId,
                limit: 20,
    ...
    static async checkUsersNeedingAttention() {
        try {
            ...
            const inactiveUsers = await soulMemoryService_1.SoulMemoryService.getInactiveUsers(sevenDaysAgo);
            for (const user of inactiveUsers) {
                await this.triggerInactiveUserWorkflow(user);
            }
            // Get users with unresolved stuck patterns
            const usersWithPatterns = await soulMemoryService_1.SoulMemoryService.getUsersWithUnresolvedPatterns();
    ...
    // ===============================================
    static async detectBreakthrough(userMessage, oracleResponse) {
        ...
        if (message.length > 200)
            confidence += 0.1; // Longer, more thoughtful responses
        if (message.includes('thank you'))
            confidence += 0.1; // Gratitude often follows breakthroughs
    ...
    // ===============================================
    static async detectRecurringPatterns(memories) {
        const patterns = [];
        ...
        for (const [patternType, keywords] of Object.entries(patternTypes)) {
            ...
            if (matchingMemories.length >= 3) {
                patterns.push({
                    ...
                    keywords: keywords.filter(k => matchingMemories.some(m => m.content.toLowerCase().includes(k)))
                });
            }
        }
        return patterns;
    }
    static getSuggestedIntervention(patternType) {
        const interventions = {
        ...
        return interventions[patternType] || 'oracle_mode_sage';
    }
    // ===============================================
    // SACRED MOMENT DETECTION
    // ===============================================
    static async detectSacredMoments(userMessage, emotionalResonance) {
        const sacredMoments = [];
        const message = userMessage.toLowerCase();
        // Gratitude moments
    ...
    // ===============================================
    static async triggerBreakthroughWorkflow(data) {
        return this.sendWebhook('/breakthrough', data);
    }
    static async triggerStuckPatternWorkflow(data) {
        return this.sendWebhook('/stuck-pattern', data);
    }
    static async triggerSacredMomentWorkflow(data) {
        return this.sendWebhook('/sacred-moment', data);
    }
    static async triggerInactiveUserWorkflow(userData) {
        return this.sendWebhook('/inactive-user', userData);
    }
    static async triggerStuckUserWorkflow(userData) {
        return this.sendWebhook('/stuck-user', userData);
    }
    static async sendWebhook(endpoint, data) {
        try {
...
exports.AutomationTriggers = AutomationTriggers;
AutomationTriggers.N8N_WEBHOOK_URL = process.env.N8N_WEBHOOK_URL || 'http://localhost:5678/webhook';
AutomationTriggers.WEBHOOK_SECRET = process.env.N8N_WEBHOOK_SECRET;
// ===============================================
// INTEGRATION HOOKS
 ...
 */
const analyzeConversationForAutomation = async (conversationData) => {
...
exports.analyzeConversationForAutomation = analyzeConversationForAutomation;
/**
 * Hook: Call when user signs up
 * Add this to your user registration handler
 */
const triggerUserSignupAutomation = async (userData) => {
    try {
        const response = await axios_1.default.post(`${process.env.API_URL}/api/automation/webhook/user-signup`, {
            ...userData,
            timestamp: new Date().toISOString()
        }, {
            headers: {
...
exports.triggerUserSignupAutomation = triggerUserSignupAutomation;
