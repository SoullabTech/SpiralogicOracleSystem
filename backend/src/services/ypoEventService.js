...
Founder, Soullab`;
    ...
    async quickElementalAssessment(participantId, responses) {
        ...
        if (responses.leadershipStyle === 'visionary')
            scores.fire += 3;
        if (responses.leadershipStyle === 'collaborative')
            scores.water += 3;
        if (responses.leadershipStyle === 'operational')
            scores.earth += 3;
        if (responses.leadershipStyle === 'strategic')
            scores.air += 3;
        if (responses.leadershipStyle === 'transformational')
            scores.aether += 3;
        // Current challenge
        if (responses.currentChallenge.includes('innovation'))
            scores.fire += 2;
        if (responses.currentChallenge.includes('team'))
            scores.water += 2;
        if (responses.currentChallenge.includes('execution'))
            scores.earth += 2;
        if (responses.currentChallenge.includes('strategy'))
            scores.air += 2;
        if (responses.currentChallenge.includes('purpose'))
            scores.aether += 2;
        // Desired outcome
        if (responses.desiredOutcome.includes('breakthrough'))
            scores.fire += 2;
        if (responses.desiredOutcome.includes('connection'))
            scores.water += 2;
        if (responses.desiredOutcome.includes('stability'))
            scores.earth += 2;
        if (responses.desiredOutcome.includes('clarity'))
            scores.air += 2;
        if (responses.desiredOutcome.includes('integration'))
            scores.aether += 2;
    ...
    getYPOElementInterpretation(element) {
    ...
    async createGroupExperience(participantIds) {
        ...
        return {
            elementalDistribution: Object.entries(elementalGroups).map(([element, members]) => ({
                ...
                percentage: Math.round((members.length / participants.length) * 100)
            })),
    ...
    async generateGroupInsights(elementalGroups) {
        const insights = [];
        ...
        // Check for balance
        const hasAllElements = Object.values(elementalGroups).every((members) => members.length > 0);
        if (hasAllElements) {
    ...
    getGroupRecommendations(elementalGroups) {
        ...
        const elementCounts = Object.entries(elementalGroups)
            .map(([element, members]) => ({ element, count: members.length }))
    ...
    getDailyPracticeForElement(element) {
        const practices = {
            fire: {
