...
describe('memoryService', () => {
    ...
    it('should store a new memory', async () => {
        ...
        supabase.from.mockReturnValue({ insert: mockInsert, single: jest.fn().mockResolvedValue({ data: { id: memoryId, content: 'test' }, error: null }) });
    ...
    it('should recall memories', async () => {
        ...
        supabase.from.mockReturnValue({ select: jest.fn().mockResolvedValue({ data: memories, error: null }) });
        const result = await memoryService.recall(userId);
        expect(result).toEqual(memories);
    });
    it('should return empty array if recall fails', async () => {
        supabase.from.mockReturnValue({ select: jest.fn().mockResolvedValue({ data: null, error: { message: 'fail' } }) });
        const result = await memoryService.recall(userId);
        expect(result).toEqual([]);
    });
    it('should update memory successfully', async () => {
        supabase.from.mockReturnValue({ update: jest.fn().mockReturnValue({ eq: jest.fn().mockResolvedValue({ error: null }) }) });
        const success = await memoryService.update(memoryId, 'new content');
        expect(success).toBe(true);
    });
    it('should return false if update fails', async () => {
        supabase.from.mockReturnValue({ update: jest.fn().mockReturnValue({ eq: jest.fn().mockResolvedValue({ error: { message: 'fail' } }) }) });
        const success = await memoryService.update(memoryId, 'new content');
        expect(success).toBe(false);
    });
    it('should delete memory successfully', async () => {
        supabase.from.mockReturnValue({ delete: jest.fn().mockReturnValue({ eq: jest.fn().mockResolvedValue({ error: null }) }) });
        const success = await memoryService.delete(memoryId);
        expect(success).toBe(true);
    });
    it('should return false if delete fails', async () => {
        supabase.from.mockReturnValue({ delete: jest.fn().mockReturnValue({ eq: jest.fn().mockResolvedValue({ error: { message: 'fail' } }) }) });
        const success = await memoryService.delete(memoryId);
        expect(success).toBe(false);
    });
});
