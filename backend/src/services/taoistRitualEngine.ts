  ...
  Wood: [
    {
      ...
      instruction:
        "Smile to your liver, visualize green healing light, stretch gently",
      element: "Wood",
      type: "meditation",
    },
    {
      ...
      instruction:
        "Stand with arms embracing invisible tree, breathe into liver meridian",
      element: "Wood",
      type: "qigong",
    },
  ],
  Fire: [
    {
      ...
      instruction:
        "Open heart center, circulate warm red energy, smile from heart",
      element: "Fire",
      type: "qigong",
    },
    {
  ...
  Earth: [
    {
      ...
      instruction:
        "Circulate energy from tailbone up spine to crown, down front to navel",
      element: "Earth",
      type: "meditation",
    },
    {
  ...
  Metal: [
    {
      ...
      instruction:
        'Exhale "Tsuuuu" sound while visualizing white light in lungs',
      element: "Metal",
      type: "sound",
    },
    {
 ...
 */
export function suggestTaoistRituals(
  profile: FourPillarsProfile,
): RitualSuggestion[] {
  // Primary focus: balance deficient elements
  const deficientRituals = profile.deficient.flatMap(
    (element) => ELEMENT_RITUAL_MAP[element] || [],
  );
  ...
  const balancingRituals = allElements
    .filter(
      (element) =>
        !profile.dominant.includes(element) &&
        !profile.deficient.includes(element),
    )
...
export function getDailyRitual(profile: FourPillarsProfile): RitualSuggestion {
  ...
  if (hour >= 3 && hour < 7)
    timeElement = "Wood"; // Liver/Gallbladder time
  else if (hour >= 11 && hour < 15)
    timeElement = "Fire"; // Heart/Small Intestine
  else if (hour >= 7 && hour < 11)
    timeElement = "Earth"; // Stomach/Spleen
  else if (hour >= 15 && hour < 19)
    timeElement = "Metal"; // Lung/Large Intestine
  else timeElement = "Water"; // Kidney/Bladder time

  // Prioritize deficient elements, otherwise use time element
  const targetElement =
    profile.deficient.length > 0 ? profile.deficient[0] : timeElement;
...
export function getSeasonalRitual(): RitualSuggestion {
  ...
  if (month >= 2 && month <= 4)
    seasonElement = "Wood"; // Spring
  else if (month >= 5 && month <= 7)
    seasonElement = "Fire"; // Summer
  else if (month >= 8 && month <= 10)
    seasonElement = "Metal"; // Autumn
 ...
 */
export function generateRitualSequence(
  profile: FourPillarsProfile,
  duration: number,
): RitualSuggestion[] {
