...
export class SoulMemoryQueryService {
  ...
  async getMemoriesByElement(userId: string, element: string, limit: number = 20): Promise<MemoryItem[]> {
    return this.getMemories({ userId, element, limit });
  }

  async searchMemories(userId: string, searchTerm: string, limit: number = 20): Promise<MemoryItem[]> {
    try {
      const { data, error } = await supabase
        .from('memories')
        .select('*')
  ...
  async getSpiritualThemes(userId: string): Promise<SpiritualTheme[]> {
    try {
      ...
      for (const memory of data || []) {
        ...
        if (Array.isArray(symbols)) {
          for (const symbol of symbols) {
            const current = themeMap.get(symbol) || { count: 0, lastSeen: timestamp };
            themeMap.set(symbol, {
              count: current.count + 1,
              lastSeen: current.lastSeen > timestamp ? current.lastSeen : timestamp
            });
          }
        }
      }
      ...
      throw new Error(`Themes retrieval failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
