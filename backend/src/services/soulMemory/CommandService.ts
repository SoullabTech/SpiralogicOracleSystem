...
export class SoulMemoryCommandService {
  ...
  async createMemory(command: CreateMemoryCommand): Promise<{ id: string; success: boolean }> {
    try {
      ...
      logger.info('Memory created successfully', { memoryId, userId: command.userId });
  ...
  async updateMemory(command: UpdateMemoryCommand): Promise<{ success: boolean }> {
    try {
      ...
      logger.info('Memory updated successfully', { memoryId: command.id, userId: command.userId });
      return { success: true };
    } catch (error) {
  ...
  async deleteMemory(command: DeleteMemoryCommand): Promise<{ success: boolean }> {
    try {
      ...
      logger.info('Memory deleted successfully', { memoryId: command.id, userId: command.userId });
      return { success: true };
    } catch (error) {
