...
export type AstrologyResponse = NatalChart | TransitReading | CompatibilityReading;

export class AstrologyService {
  private readonly SIGNS = ['Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 
    'Libra', 'Scorpio', 'Sagittarius', 'Capricorn', 'Aquarius', 'Pisces'];
  
  private readonly PLANETS = ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 
  ...
  async getNatalChart(query: AstrologyQuery): Promise<StandardAPIResponse<NatalChart>> {
    try {
      logger.info('Generating natal chart', { userId: query.userId });

      if (!query.birthData?.date) {
        return errorResponse(['Birth date is required for natal chart calculation']);
      }

      const natalChart = await this.calculateNatalChart(query.userId, query.birthData);
  ...
  async getCurrentTransits(query: AstrologyQuery): Promise<StandardAPIResponse<TransitReading>> {
    try {
      logger.info('Calculating current transits', { userId: query.userId });

      if (!query.birthData?.date) {
        return errorResponse(['Birth data is required for transit calculation']);
      }

      const transits = await this.calculateTransits(query.birthData, query.currentDate);
  ...
  async getCompatibilityReading(query: AstrologyQuery): Promise<StandardAPIResponse<CompatibilityReading>> {
    try {
      logger.info('Calculating compatibility', { userId: query.userId });

      if (!query.birthData?.date || !query.partnerBirthData?.date) {
        return errorResponse(['Both birth dates are required for compatibility reading']);
      }

      const compatibility = await this.calculateCompatibility(query.birthData, query.partnerBirthData);
  ...
  private async calculateNatalChart(userId: string, birthData: BirthData): Promise<NatalChart> {
    ...
    const interpretation = {
      ...
      spiralogicGuidance: this.generateSpiralogicGuidance(elementalBalance, sunSign, moonSign)
    };
    ...
    return {
      ...
      midheaven: { sign: this.SIGNS[Math.floor(Math.random() * 12)], degree: Math.floor(Math.random() * 30) },
      moonNodes: {
        north: { sign: this.SIGNS[Math.floor(Math.random() * 12)], degree: Math.floor(Math.random() * 30), house: Math.floor(Math.random() * 12) + 1 },
        south: { sign: this.SIGNS[Math.floor(Math.random() * 12)], degree: Math.floor(Math.random() * 30), house: Math.floor(Math.random() * 12) + 1 }
      },
      interpretation
    };
  }

  private async calculateTransits(birthData: BirthData, currentDate?: string): Promise<TransitReading> {
    ...
    const elementalInfluence = this.calculateCurrentElementalInfluence(activeTransits);
    
    return {
      date,
      activeTransits,
      dailyGuidance: 'Focus on balancing expansion with practical grounding today.',
      elementalInfluence,
      recommendation: 'Embrace new opportunities while maintaining emotional stability.'
    };
  }

  private async calculateCompatibility(birthData1: BirthData, birthData2: BirthData): Promise<CompatibilityReading> {
    ...
    return {
      ...
      challenges: ['Different communication styles', 'Timing of emotional needs'],
  ...
  private calculateAspects(planets: Planet[]): Aspect[] {
    ...
    for (let i = 0; i < planets.length; i++) {
      for (let j = i + 1; j < planets.length; j++) {
        ...
        if (Math.random() > 0.7) { // 30% chance of significant aspect
          aspects.push({
            planet1: planets[i].name,
            planet2: planets[j].name,
            type: ['conjunction', 'trine', 'square', 'opposition', 'sextile'][Math.floor(Math.random() * 5)] as any,
            orb: Math.random() * 8,
  ...
  private calculateCurrentElementalInfluence(transits: any[]): ElementalBalance {
  ...
  private generateDominantThemes(balance: ElementalBalance): string[] {
    ...
    return themes[balance.dominantElement as keyof typeof themes] || themes.fire;
  }
  ...
  private generateChallenges(balance: ElementalBalance): string[] {
    ...
    return challenges[balance.dominantElement as keyof typeof challenges] || challenges.fire;
  }
  ...
  private generateStrengths(balance: ElementalBalance): string[] {
    ...
    return strengths[balance.dominantElement as keyof typeof strengths] || strengths.fire;
  }
  ...
  private generateSpiralogicGuidance(balance: ElementalBalance, sunSign: string, moonSign: string): string {
