 ...
 */
export async function logAgentInteraction({ userId, agent, content, phase, }) {
 ...
 */
export async function logJournalEntry({ userId, content, emotionTag, symbols, }) {
 ...
 */
export async function logAdjusterInsight({ userId, content, phase, }) {
    const { error } = await supabase.from('adjuster_logs').insert({
        user_id: userId,
        agent: 'AdjusterAgent',
        message: content,
