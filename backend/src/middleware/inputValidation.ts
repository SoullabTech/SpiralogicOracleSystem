...
import { OracleErrorResponseBuilder, OracleErrorCodes } from '../types/oracleResponse';
...
export const commonSchemas = {
  ...
  element: z.enum(['fire', 'water', 'earth', 'air', 'aether'], {
    errorMap: () => ({ message: 'Element must be one of: fire, water, earth, air, aether' })
  }),
 ...
 */
export function validateInput(schema: ZodSchema, source: 'body' | 'query' | 'params' = 'body') {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (error instanceof ZodError) {
        const errorResponse = OracleErrorResponseBuilder
          .create()
...
export function validateContentSecurity(req: Request, res: Response, next: NextFunction) {
  ...
  if (req.method !== 'GET' && req.method !== 'HEAD' && !allowedTypes.some(type => contentType.includes(type))) {
    const errorResponse = OracleErrorResponseBuilder
      .create()
    ...
    logger.warn('Invalid content type blocked', { contentType, path: req.path });
...
export function validateRequestSize(maxSizeBytes: number = 1024 * 1024) { // 1MB default
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (contentLength > maxSizeBytes) {
      ...
      logger.warn('Request size exceeded', { contentLength, maxSizeBytes, path: req.path });
...
export function validateUserAgent(req: Request, res: Response, next: NextFunction) {
  ...
  if (isSuspicious) {
    logger.warn('Suspicious user agent detected', { userAgent, ip: req.ip, path: req.path });
