...
export const cacheMiddleware = (configKey?: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const config = configKey ? cacheConfigs[configKey] : cacheConfigs[req.path];
...
export const cacheInvalidationMiddleware = () => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const pathsToInvalidate = Object.entries(cacheConfigs)
      .filter(([path, config]) => {
        return (
          config.invalidateOn?.includes(req.method) &&
          req.path.startsWith(path.split(":")[0])
      ...
      res.json = function (data: any) {
        ...
        pathsToInvalidate.forEach((prefix) => {
          CacheManager.invalidatePattern(`${prefix}:*`)
            .then((count) =>
              logger.debug(`Invalidated ${count} cache entries for ${prefix}`),
            )
