...
async function testSoulMemoryIntegration() {
    ...
    const searchResults = await soulMemory.semanticSearch('test_user_123', 'overwhelmed changes', { topK: 3 });
    ...
    const ritualMemory = await soulMemory.recordRitualMoment('test_user_123', 'morning_intention', 'Setting intention to embrace change with grace', 'water', await oracle.generateRitualGuidance('morning_intention'));
    ...
    const thread = await soulMemory.createMemoryThread('test_user_123', 'Integration Journey', 'integration');
