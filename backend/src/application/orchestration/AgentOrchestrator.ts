...
import { FireAgent } from "../../domain/agents/elemental/fireAgent";
import { WaterAgent } from "../../domain/agents/elemental/waterAgent";
import {
  MayaPromptProcessor,
  MayaPromptContext,
...
export class AgentOrchestrator {
  ...
  async processQuery(
    ...
    const responses = await this.gatherAgentResponses(
      input,
      intent,
      userContext,
      strategy,
    ...
    const synthesis = await this.responseSynthesizer.synthesize(
      responses,
      intent,
      strategy,
    );
  ...
  private determineOrchestrationStrategy(
    ...
    const recentFireCount = recentHistory.filter(
      (h) => h.primary === "fire",
    ).length;
    const recentWaterCount = recentHistory.filter(
      (h) => h.primary === "water",
    ).length;
  ...
  async getArchetypalInsights(userContext?: UserContext) {
    ...
    if (!sessionMemory) {
      return {
        ...
        dominantPatterns: [],
        emergentWisdom:
          "Your journey is just beginning. Both fire and water energies await your exploration.",
      };
    }
    ...
    const avgFire =
      patterns.reduce((sum, p) => sum + p.balance.fire, 0) / patterns.length;
    const avgWater =
      patterns.reduce((sum, p) => sum + p.balance.water, 0) / patterns.length;

    // Identify dominant patterns
    const agentCounts = patterns.reduce(
      (counts, p) => {
    ...
    const emergentWisdom = this.generateEmergentWisdom(
      avgFire,
      avgWater,
      dominantPatterns,
    );
  ...
  private generateEmergentWisdom(
    fireBalance: number,
    waterBalance: number,
    patterns: any[],
  ): string {
...
class ResponseSynthesizer {
  async synthesize(
    ...
    const archetypalBalance = this.calculateArchetypalBalance(
      fire,
      water,
      intent,
    );
    ...
    return {
      ...
      metadata: {
        ...
        transformationGoal: this.determineTransformationGoal(
          fire,
          water,
          intent,
        ),
  ...
  private calculateArchetypalBalance(
    fire: any,
    water: any,
    intent: ArchetypalIntent,
  ) {
    ...
    let fireWeight =
      intent.primary === "fire" ? intent.confidence : 1 - intent.confidence;
    let waterWeight =
      intent.primary === "water" ? intent.confidence : 1 - intent.confidence;
  ...
  private generateCollectiveWisdom(
    fire: any,
    water: any,
    intent: ArchetypalIntent,
  ): string {
    const wisdomTemplates = {
      fire_dominant:
        "Your fire energy is strong right now. Let your emotions fuel rather than overwhelm your vision.",
      water_dominant:
        "Your emotional wisdom is flowing. Trust how it can activate and direct your creative power.",
      balanced:
        "You're integrating both vision and feeling beautifully. This is the path of wholeness.",
      evolving:
        "I see both your creative fire and emotional depth evolving. Trust this archetypal dance.",
    };
