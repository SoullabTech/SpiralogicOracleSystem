...
router.get('/memories', async (req, res) => {
    try {
        ...
        const { type, element, limit = '20', sacred, transformations, retreatMode, retreatPhase, startDate, endDate } = req.query;
        const options = {
        ...
        if (retreatMode === 'true') {
            filteredMemories = memories.filter(memory => memory.metadata?.mode === 'retreat' ||
                memory.metadata?.retreatPhase ||
                memory.metadata?.retreatIntensive ||
                memory.ritualContext?.includes('retreat'));
        }
        if (retreatPhase) {
            filteredMemories = filteredMemories.filter(memory => memory.metadata?.retreatPhase === retreatPhase);
        }
        res.json({
            success: true,
