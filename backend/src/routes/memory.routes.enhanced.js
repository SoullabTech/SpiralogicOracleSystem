...
router.post('/', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [traditionalMemory, soulMemory] = await Promise.all([
            memoryService.store(userId, parse.data.content, parse.data.element, parse.data.sourceAgent, parse.data.confidence, parse.data.metadata),
            parse.data.type ?
...
router.post('/journal', asyncHandler(async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ error: 'Unauthenticated' });
    try {
        const result = await memoryIntegrationService.storeJournalEntryIntegrated(userId, parse.data.content, parse.data.symbols || [], {
            element: parse.data.element,
 ...
 */
router.get('/sacred-moments', asyncHandler(async (req, res) => {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ error: 'Unauthenticated' });
 ...
 */
router.get('/transformation-journey', asyncHandler(async (req, res) => {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ error: 'Unauthenticated' });
    try {
...
router.post('/search', asyncHandler(async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ error: 'Unauthenticated' });
    try {
        const results = await memoryIntegrationService.searchUnifiedMemory(userId, parse.data.query, {
 ...
 */
router.post('/oracle-query', asyncHandler(async (req, res) => {
 ...
 */
router.get('/archetypes', asyncHandler(async (req, res) => {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ error: 'Unauthenticated' });
    try {
