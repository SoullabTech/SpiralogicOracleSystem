...
facilitatorDashboardRouter.get('/participants', authenticate, facilitatorAuth, async (req, res) => {
    try {
      ...
      `)
        ...
        // Apply filters
        if (filter === 'active') {
            query = query.gte('last_session.scheduled_time', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString());
        }
        // Apply sorting
...
async function getTransformationTimeline(participantId) {
    // Would aggregate various events into timeline
    return [];
}
async function scheduleFollowUpActions(sessionId, actions, facilitatorId) {
