...
const soulMemoryService_1 = require("../services/soulMemoryService");
 ...
 */
router.post('/webhook/user-signup', validateWebhookSignature, async (req, res) => {
    try {
 ...
 */
router.post('/webhook/breakthrough', validateWebhookSignature, async (req, res) => {
    try {
        const { userId, breakthroughType, confidenceScore, conversationId, breakthroughMarkers, timestamp } = req.body;
        ...
        const userMemories = await soulMemoryService_1.SoulMemoryService.retrieve({ userId, limit: 10 });
 ...
 */
router.post('/webhook/sacred-moment', validateWebhookSignature, async (req, res) => {
    try {
        const { userId, momentType, content, emotionalResonance, timestamp } = req.body;
        // Store sacred moment
        await soulMemoryService_1.SoulMemoryService.store({
            userId,
            type: 'sacred_moment',
 ...
 */
router.post('/webhook/stuck-pattern', validateWebhookSignature, async (req, res) => {
    try {
        const { userId, patternType, occurrenceCount, suggestedIntervention, lastOccurrences } = req.body;
 ...
 */
router.get('/metrics/daily', authenticateToken_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.get('/users/needs-attention', authenticateToken_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.get('/status', authenticateToken_1.authenticateToken, async (req, res) => {
    try {
        const status = {
 ...
 */
function generateBreakthroughMessage(breakthroughType) {
    const messages = {
    ...
    return messages[breakthroughType] || 'Something sacred shifted in your consciousness.';
}
/**
...
async function generateDailyMetrics(date) {
    try {
        ...
        const memories = await soulMemoryService_1.SoulMemoryService.getMemoriesByDateRange(today.toISOString(), tomorrow.toISOString());
        const metrics = {
...
async function getUsersNeedingAttention() {
    try {
        ...
        const concerningPatterns = await soulMemoryService_1.SoulMemoryService.getUsersWithPattern('pattern_recognition');
        const stuckUsers = concerningPatterns.filter(user => user.patternCount >= 5 || user.lastActivity > 7 // days ago
        );
