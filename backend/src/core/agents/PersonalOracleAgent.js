...
const SacredVoiceProtocols = {
    ...
    sacredMirror: {
        beforeEveryResponse: async (input, potentialResponse) => {
            const checks = {
...
class PersonalOracleAgent extends baseAgent_js_1.BaseAgent {
    constructor(config) {
        ...
        // Oracle Mode System
        this.currentOracleMode = 'sage'; // Default to balanced mode
        this.modeHistory = [];
        this.modeSuggestionCooldown = null;
        // Jung-Buddha Sacred Mirror System
        this.sacredMirrorMode = 'adaptive';
        this.jungArchetypeHistory = [];
        this.buddhaAttachmentPatterns = [];
        this.integrationLiberationBalance = 0.5; // 0 = full liberation, 1 = full integration
...
What brought you here today? Not just the immediate reason, but the deeper current that carried you to this moment?`;
    ...
    async applySacredMirrorProtocol(prompt, context) {
    ...
    determineResponseType(prompt, context, mirrorCheck) {
        if (mirrorCheck.needsResistance)
            return 'sacred_resistance';
        if (mirrorCheck.needsDepth)
            return 'depth_invitation';
        if (mirrorCheck.needsShadowWork)
            return 'shadow_work';
        if (mirrorCheck.isInLoop)
            return 'pattern_disruption';
        if (context.needsWitnessing)
            return 'sacred_witnessing';
        if (context.integrationPhase)
            return 'integration_support';
        return 'sacred_presence';
    }
    async generateSacredResponse(prompt, context, responseType) {
    ...
    enhanceResponseWithMemoryPatterns(response, patterns) {
    ...
    enhanceResponseWithSemanticMemory(response, currentPrompt, semanticMemories) {
        ...
        const hasReference = referenceIndicators.some(indicator => currentPrompt.toLowerCase().includes(indicator));
    ...
    findMostRelevantMemory(prompt, memories) {
        ...
        memories.forEach(memory => {
            ...
            const sharedWords = promptWords.filter(word => memoryWords.includes(word) && word.length > 3);
    ...
    createEmotionalContinuityResponse(response, previousMemory) {
    ...
    addSacredResistance(response, reason) {
        const resistance = this.selectFromArray(SacredVoiceProtocols.sacredMirror.resistanceResponses);
        return `${resistance} ${response}`;
    }
    applyElementalFilter(response, context) {
    ...
    // ===============================================
    async activateRetreatMode(phase) {
        this.mode = 'retreat';
    ...
    async checkSafetyProtocols(prompt) {
    ...
    handleSafetyResponse(safetyCheck) {
    ...
    async storeExchange(prompt, response, context) {
    ...
    updateSacredRelationship(prompt, context, responseType) {
        ...
        if (responseType === 'integration_support') {
            this.sacredRelationship.transformationMilestones.push(`Integration: ${new Date().toISOString()}`);
        }
    }
    // ===============================================
    // HELPER METHODS
    // ===============================================
    isSeekingBypass(prompt, context) {
    ...
    async searchSemanticMemories(prompt) {
        ...
        try {
            ...
            const memories = await this.soulMemory.semanticSearch(this.userId, prompt, {
                topK: 5,
    ...
    detectMemoryPatterns(memories) {
        ...
        memories.forEach(memory => {
            ...
            if (memory.metadata?.themes) {
                memory.metadata.themes.forEach((theme) => {
    ...
    async storeMemoryFromExchange(prompt, response, context) {
        if (!this.soulMemory)
            return;
        ...
        if (isBreakthrough) {
            this.sacredRelationship.transformationMilestones.push(`Breakthrough: ${prompt.substring(0, 50)}... - ${new Date().toISOString()}`);
        }
    ...
    async recordSacredMoment(content, type = 'sacred_pause') {
        if (!this.soulMemory)
            return;
        await this.soulMemory.storeMemory({
            userId: this.userId,
    ...
    async switchOracleMode(newMode, reason, triggeredBy = 'user_choice') {
    ...
    async suggestModeSwitch(context) {
    ...
    applyTaoFilter(response, context) {
    ...
    analyzeConversationContext(prompt, context) {
        return {
    ...
    detectSpiritualBypass(prompt, context) {
    ...
    // ===============================================
    async generateRitualGuidance(ritualType) {
...
Feel the warmth of appreciation spreading through your being.`
    ...
    async reflectOnJournal(journalEntry) {
        if (!this.soulMemory) {
    ...
    generateBasicJournalReflection(entry) {
    ...
    // ===============================================
    detectBreakthrough(prompt, context) {
        ...
        const hasBreakthroughLanguage = breakthroughIndicators.some(indicator => lowerPrompt.includes(indicator));
        ...
        const representsShift = context.memoryPatterns?.repeatedThemes?.length > 0 &&
            !context.memoryPatterns.repeatedThemes.some((theme) => lowerPrompt.includes(theme.theme.toLowerCase()));
        return hasBreakthroughLanguage || discussingPatterns ||
            (highEmotionalIntensity && representsShift);
    }
    detectSacredContent(prompt) {
    ...
    detectEmotionalContent(prompt) {
        ...
        const emotionalWordCount = emotionalWords.filter(word => lowerPrompt.includes(word)).length;
        return Math.min(emotionalWordCount / wordCount * 10, 1);
    }
    // ===============================================
    // ARCHETYPAL PATTERN DETECTION
    // ===============================================
    detectArchetype(prompt, context) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    async checkArchetypalAdaptation(archetype) {
        if (!this.soulMemory)
            return;
    ...
    detectRetreatIntensive(prompt) {
        ...
        const hasRetreatLanguage = retreatIntensiveIndicators.some(indicator => lowerPrompt.includes(indicator));
    ...
    // ===============================================
    determineJungBuddhaMode(prompt, context, responseType) {
        // Override mode if set explicitly
        if (this.sacredMirrorMode !== 'adaptive') {
            return this.sacredMirrorMode;
        }
    ...
    // ===============================================
    async determineWisdomApproach(prompt, context) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    detectAvoidanceLanguageSimple(prompt) {
    ...
    needsWisdomBalance(recentMemories) {
    ...
    getPredominantWisdomMode(recentMemories) {
        const wisdomModeCount = this.countWisdomModes(recentMemories);
        if (wisdomModeCount.jung > wisdomModeCount.buddha)
            return 'jung';
        if (wisdomModeCount.buddha > wisdomModeCount.jung)
            return 'buddha';
        return 'hybrid';
    }
    countWisdomModes(memories) {
        const count = { jung: 0, buddha: 0, hybrid: 0 };
        memories.forEach(memory => {
            if (memory.wisdomMode) {
                count[memory.wisdomMode]++;
            }
        });
        return count;
    }
    // Public method to get current simple wisdom mode
    getCurrentWisdomMode() {
    ...
    updateSystemPromptForMode(mode) {
    ...
    async suggestModeForInput(userInput) {
    ...
    buildUserContextForWisdomEngine(prompt, context) {
    ...
    updateRecentPatterns(prompt, context) {
        const patterns = [];
    ...
    mapEmotionalState(context) {
        if (context.crisisMarkers?.length > 0)
            return 'crisis';
        if (context.responseType === 'integration_support')
            return 'integration';
        if (context.shadowThemes?.length > 0)
            return 'transformation';
        if (context.emotional_depth > 0.8)
            return 'chaos';
        return 'stable';
    }
    calculateAttachmentLevel(context) {
    ...
    calculateShadowReadiness(context) {
    ...
    detectAvoidanceLanguage(prompt) {
        const avoidanceIndicators = [
    ...
    detectIdentityQuestions(prompt) {
    ...
    calculateGraspingIntensity(prompt) {
    ...
    calculateAvoidanceIntensity(prompt) {
    ...
    async suggestSacredPracticeForPattern(patternType) {
        const practices = {
            grasping: {
                primary: 'friday',
                secondary: 'sunday',
    ...
    async recordSacredWeeklyMoment(dayPractice, experience) {
        if (!this.soulMemory)
            return;
    ...
    generateJungShadowResponse(prompt, context) {
    ...
    generateBuddhaLiberationResponse(prompt, context) {
        const protocols = SacredVoiceProtocols.sacredMirror;
    ...
    generateHybridIntegrationResponse(prompt, context) {
        const protocols = SacredVoiceProtocols.sacredMirror;
    ...
    detectAngerPattern(prompt) {
        ...
        let intensity = 0;
        if (lowerPrompt.includes('hate how angry') || lowerPrompt.includes('hate being angry'))
            intensity = 0.9;
    ...
    detectFearPattern(prompt) {
        ...
        let intensity = 0;
        if (lowerPrompt.includes('hate being afraid') || lowerPrompt.includes('hate feeling scared'))
            intensity = 0.9;
        else if (hasFear && lowerPrompt.includes('hate'))
            intensity = 0.7;
        else if (hasFear)
            intensity = 0.5;
        return { isPresent: hasFear, isRejecting, intensity };
    }
    detectShamePattern(prompt) {
        const lowerPrompt = prompt.toLowerCase();
        ...
        let intensity = 0;
        if (lowerPrompt.includes('hate myself') || lowerPrompt.includes('ashamed of who'))
            intensity = 0.9;
        else if (hasShame && (lowerPrompt.includes('hate') || lowerPrompt.includes('disgusted')))
            intensity = 0.7;
        else if (hasShame)
            intensity = 0.5;
        return { isPresent: hasShame, isRejecting, intensity };
    }
    detectSadnessPattern(prompt) {
        const lowerPrompt = prompt.toLowerCase();
        ...
        let intensity = 0;
        if (lowerPrompt.includes('hate feeling sad') || lowerPrompt.includes('sick of being sad'))
            intensity = 0.9;
        else if (hasSadness && lowerPrompt.includes('hate'))
            intensity = 0.7;
        else if (hasSadness)
            intensity = 0.5;
        return { isPresent: hasSadness, isRejecting, intensity };
    }
    detectIdentityOverAttachment(prompt) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    detectEmotionIdentification(prompt) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    detectRecurringPattern(prompt) {
        ...
        if (!isRecurring)
            return { emotion: '', isRecurring: false };
    ...
    // ===============================================
    generateModeSpecificResponse(prompt, context) {
    ...
    getModeResponseForTheme(theme, prompt) {
        const responses = {
            mother_relationship: {
    ...
    applyJungBuddhaWisdom(baseResponse, mode, prompt, context) {
    ...
    detectJungArchetype(prompt, context) {
        const lowerPrompt = prompt.toLowerCase();
        // Detect active Jungian archetypes beyond the basic ones
        if (/mask|persona|pretend|image/.test(lowerPrompt))
            return 'Persona';
        if (/dark|shadow|hate|reject|ashamed/.test(lowerPrompt))
            return 'Shadow';
        ...
        if (/whole|complete|integrated|balanced/.test(lowerPrompt))
            return 'Self';
        if (/mother|nurture|care|protect/.test(lowerPrompt))
            return 'Great Mother';
        if (/father|authority|structure|discipline/.test(lowerPrompt))
            return 'Great Father';
        if (/trick|clever|mischief|boundary/.test(lowerPrompt))
            return 'Trickster';
        return null;
    }
    ...
    detectAttachmentPatterns(prompt) {
        ...
        if (/deserve|earn|worthy/.test(lowerPrompt))
            patterns.push('worthiness');
        return patterns;
    }
    // Public methods for Jung-Buddha mode management
    async setSacredMirrorMode(mode) {
    ...
    async adjustIntegrationLiberationBalance(direction) {
