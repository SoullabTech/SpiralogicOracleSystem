...
const ShadowIntelligence = {
    detectShadowType: (input, context) => {
    ...
    craftShadowResponse: (input, shadowType, memories) => {
...
What in your life keeps triggering the same response? What pattern keeps playing out? What part of your humanity are you trying to reject? Your greatest gifts often hide in what you judge most harshly about yourself.`;
    ...
    addShadowSignature: (response, shadowType) => {
        const signatures = {
...
class ShadowAgent extends oracleAgent_1.OracleAgent {
    constructor() {
        super({ debug: false });
    }
...
${baseResponse.response}`;
    ...
    assessIntegrationPotential(input) {
    ...
    assessShadowConfidence(input, shadowType) {
    ...
    extractIntegrationInvitations(content) {
        const invitations = [];
