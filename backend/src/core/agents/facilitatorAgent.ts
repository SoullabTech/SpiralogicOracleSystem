...
export class FacilitatorAgent extends OracleAgent {
  ...
  public async proposeIntervention(userId: string): Promise<string> {
    try {
      ...
      logger.info("Facilitator intervention generated", {
        userId,
        element: topElement,
      });
  ...
  private getRitualForElement(element: string): string {
    const rituals: Record<string, string> = {
      ...
      water:
        "💧 Submerge your hands in water. Let emotion flow. Journal what arises.",
      earth:
        "🌿 Walk barefoot. Feel gravity’s truth. What do you need to plant?",
    ...
    return (
      rituals[element] || "Take a moment in stillness. Let intuition rise."
    );
  }
}
