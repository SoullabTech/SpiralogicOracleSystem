...
export class DualWisdomDemo {
  ...
  private async testInput(
    ...
    if (sophisticatedRouting) {
      console.log(
        `🧠 Sophisticated Detection: ${sophisticatedRouting.approach} (confidence: ${sophisticatedRouting.confidence.toFixed(2)})`,
      );
      console.log(`💡 Reasoning: ${sophisticatedRouting.reasoning}`);

      if (sophisticatedRouting.adjustments) {
        console.log(
          `⚙️ Adjustments: ${JSON.stringify(sophisticatedRouting.adjustments)}`,
        );
      }
      ...
      if (!agreement) {
        console.log(
          `📊 Confidence Impact: ${sophisticatedRouting.confidence < 0.6 ? "Simple mode used due to low confidence" : "Sophisticated mode used"}`,
        );
      }
    } else {
  ...
  async demonstrateWisdomBalancing() {
    ...
    const conversationSequence = [
      {
        input: "I am so attached to this outcome",
        expectedBalance: "Buddha for attachment",
      },
      {
        input: "I am desperate for this to work",
        expectedBalance: "Buddha for desperation",
      },
      { input: "I must have this", expectedBalance: "Buddha for grasping" },
      {
        input: "I need to control everything",
      ...
      {
        input: "This feeling is too much",
        expectedBalance: "Could be Buddha for spaciousness",
      },
    ];
    ...
    const edgeCases = [
      {
        ...
        expected:
          "Systems may disagree - simple might pick one, sophisticated considers both",
      },
      {
