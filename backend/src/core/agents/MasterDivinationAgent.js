...
export class MasterDivinationAgent extends BaseAgent {
    ...
    async retrieveRelevantHistory(userId, system) {
        if (!this.soulMemory)
            return [];
        ...
        return memories.filter(m => m.metadata?.system === system ||
    ...
    // I CHING DIVINATION
    // ===============================================
    async divineIChing(request, history) {
        // Cast hexagram
    ...
    async interpretIChing(hexagram, request, history) {
    ...
    async generateIChingGuidance(hexagram, request, interpretation) {
    ...
    // TAROT DIVINATION
    // ===============================================
    async divineTarot(request, history) {
        // Draw spread
    ...
    async interpretTarot(spread, request, history) {
    ...
    Cards drawn:
    ${spread.cards.map((c) => `${c.position.name}: ${c.card.name}${c.reversed ? ' (Reversed)' : ''}`).join('\n')}
    ...
    async generateTarotGuidance(spread, request, interpretation) {
    ...
    async divineUnified(request, history) {
        ...
        const unifiedReading = {
            ...
            readings: {
                ...
                tarot: tarot.cards.map((c) => c.card.name),
    ...
    extractElements(readings) {
        const elements = [];
        readings.forEach(reading => {
            if (reading.element)
                elements.push(reading.element);
            if (reading.dominantElement)
                elements.push(reading.dominantElement);
    ...
    extractNumbers(readings) {
        const numbers = [];
        readings.forEach(reading => {
            if (reading.number)
                numbers.push(reading.number);
            if (reading.lifePathNumber)
                numbers.push(reading.lifePathNumber);
            if (reading.cards) {
    ...
    extractArchetypes(readings) {
        const archetypes = [];
        readings.forEach(reading => {
            if (reading.archetype)
                archetypes.push(reading.archetype);
            if (reading.cards) {
                reading.cards.forEach((card) => {
                    if (card.card.archetype)
                        archetypes.push(card.card.archetype);
                });
            }
        });
        return archetypes;
    ...
    // HELPER METHODS
    // ===============================================
    getElementFromReading(result) {
        if (result.elementalInfluence) {
            return result.elementalInfluence.toLowerCase();
        }
        return 'aether'; // Default
    }
    getElementalInfluence(reading) {
    ...
    distillCoreMessage(interpretation) {
    ...
    findSharedArchetypes(archetypes) {
        return this.findDominantPatterns(archetypes);
    }
    getNumberSignificance(number) {
        const significances = {
    ...
    Tarot: ${data.tarot.cards.map((c) => c.card.name).join(', ')}
    ...
    ${data.synchronicities.map((s) => `- ${s.description} (${s.significance})`).join('\n')}
    ...
    async synthesizeGuidance(data) {
        const prompt = `Based on this unified divination, offer synthesized guidance:
    
    Interpretation: ${data.interpretation}
    
    Key synchronicities: ${data.synchronicities.map((s) => s.description).join(', ')}
    ...
    // ===============================================
    async divineRunes(request, history) {
    ...
    async divineAstrology(request, history) {
    ...
    async divineNumerology(request, history) {
        // Implementation for Numerology
        const numbers = await this.systems.numerology.calculateFromQuestion(request.question, request.additionalContext?.birthDate);
        return {
            system: 'numerology',
            reading: numbers,
    ...
    async divineSacredGeometry(request, history) {
