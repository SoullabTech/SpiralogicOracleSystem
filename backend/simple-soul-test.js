...
try {
  ...
  const memories = db
    .prepare("SELECT * FROM soul_memory WHERE user_id = ?")
    .all("test_user_123");
  ...
  const totalCount = db
    .prepare("SELECT COUNT(*) as count FROM soul_memory")
    .get();
  const typeStats = db
    .prepare("SELECT type, COUNT(*) as count FROM soul_memory GROUP BY type")
    .all();
