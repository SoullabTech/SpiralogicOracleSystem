 ...
 */
export async function calculateIChingCompatibility(
  req: Request,
  res: Response,
) {
  try {
...
export async function getAllTrigrams(req: Request, res: Response) {
  try {
    const trigrams = [
      ...
      .map((name) => getTrigramArchetype(name))
      .filter(Boolean);
    ...
    res.json({
      ...
      description:
        "I Ching trigram archetypes with elemental and directional correspondences",
    });
  } catch (error) {
