...
async function runTests() {
  ...
  for (const endpoint of testEndpoints) {
    ...
    if (result.status === "ERROR") {
      ...
      try {
        ...
        console.log(
          `✅ Status ${result.status} - ${data.success ? "Success" : "Response received"}`,
        );

        // Show relevant data
        if (data.memories)
          console.log(`   📊 ${data.memories.length} memories`);
        if (data.sacredMoments)
          console.log(`   ✨ ${data.sacredMoments.length} sacred moments`);
        if (data.patterns)
          console.log(`   🎭 ${data.patterns.length} archetypal patterns`);
        if (data.journey)
          console.log(`   🌟 Journey phase: ${data.journey.currentPhase}`);
      } catch (e) {
  ...
  console.log(
    'curl "http://localhost:3001/api/soul-memory/memories?userId=test_user_123"',
  );
}
