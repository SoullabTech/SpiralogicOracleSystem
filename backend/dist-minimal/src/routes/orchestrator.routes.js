...
const orchestrator_1 = require("../core/agents/orchestrator");
...
router.post("/query", async (req, res) => {
    try {
        ...
        const result = await agentOrchestrator_1.agentOrchestrator.processQuery(input, userContext);
        res.json({
            success: true,
            data: result,
...
router.get("/insights", async (req, res) => {
    try {
        ...
        const insights = await agentOrchestrator_1.agentOrchestrator.getArchetypalInsights(userContext);
        res.json({
            success: true,
            data: insights,
            timestamp: new Date().toISOString(),
...
router.get('/workflows', async (req, res) => {
    try {
        const workflows = orchestrator_1.sacredOrchestrator.getAvailableWorkflows();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to get workflows:', error);
        res.status(500).json({
            success: false,
...
router.post('/journey/start', async (req, res) => {
    try {
        ...
        const result = await orchestrator_1.sacredOrchestrator.startJourney(workflowId, userId, context);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to start journey:', error);
        res.status(500).json({
            success: false,
...
router.post('/journey/:journeyId/next', async (req, res) => {
    try {
        ...
        const result = await orchestrator_1.sacredOrchestrator.executeNextStep(journeyId, userInput);
        res.json({
            success: true,
        ...
        logger_1.logger.error(`Failed to execute next step for journey ${req.params.journeyId}:`, error);
        res.status(500).json({
            success: false,
...
router.get('/journey/:journeyId/status', async (req, res) => {
    try {
        const { journeyId } = req.params;
        const status = orchestrator_1.sacredOrchestrator.getJourneyStatus(journeyId);
        if (!status) {
        ...
        logger_1.logger.error(`Failed to get journey status for ${req.params.journeyId}:`, error);
        res.status(500).json({
            success: false,
...
router.post('/journey/:journeyId/pause', async (req, res) => {
    try {
        const { journeyId } = req.params;
        const success = orchestrator_1.sacredOrchestrator.pauseJourney(journeyId);
        if (!success) {
            return res.status(404).json({
                success: false,
                error: 'Journey not found'
        ...
        logger_1.logger.error(`Failed to pause journey ${req.params.journeyId}:`, error);
        res.status(500).json({
            success: false,
...
router.post('/journey/:journeyId/cancel', async (req, res) => {
    try {
        const { journeyId } = req.params;
        const success = orchestrator_1.sacredOrchestrator.cancelJourney(journeyId);
        if (!success) {
            return res.status(404).json({
                success: false,
                error: 'Journey not found'
        ...
        logger_1.logger.error(`Failed to cancel journey ${req.params.journeyId}:`, error);
        res.status(500).json({
            success: false,
...
router.post('/workflow/suggest', async (req, res) => {
    try {
        ...
        const workflow = orchestrator_1.sacredOrchestrator.getWorkflowByTrigger(trigger);
        if (!workflow) {
            return res.json({
                success: true,
                workflow: null,
        ...
        logger_1.logger.error('Failed to suggest workflow:', error);
        res.status(500).json({
            success: false,
