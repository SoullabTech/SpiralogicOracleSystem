...
import {
  ...
  previewArchetypalVoice,
} from "./src/utils/voiceService.js";

console.log(
  "🎭 Testing Archetypal Voice Integration with Maya Consciousness System\n",
);
...
async function testArchetypalVoices() {
  try {
    ...
    for (const test of testArchetypes) {
      console.log(
        `🧪 Testing ${test.archetype.toUpperCase()} archetype voice:`,
      );
      ...
      try {
        ...
        console.log(
          `   ⚡ Energy: ${voiceResult.voiceMetadata.energySignature}`,
        );
        console.log("");
    ...
    console.log(
      `🎯 Individual Voice Tests: ${successfulVoices}/${totalTests} successful\n`,
    );
    ...
    for (const test of dualArchetypeTests) {
      ...
      try {
        ...
        console.log(
          `   🎭 Blended Personality: ${voiceResult.voiceMetadata.personality}`,
        );
        console.log("");
    ...
    console.log(
      `🎯 Dual Voice Tests: ${dualVoiceTests}/${dualArchetypeTests.length} successful\n`,
    );
...
async function testEnhancedHierarchyWithVoice() {
  try {
    console.log(
      "🌟 Testing Enhanced HierarchyOrchestrator with Voice Integration...\n",
    );
    ...
    for (const test of testQueries) {
      ...
      try {
        ...
        console.log(
          `   ✅ Response generated (${response.content.length} chars)`,
        );
        console.log(
          `   🔮 Primary Archetype: ${response.archetypalMetadata.primary}`,
        );
        console.log(`   🎭 Maya Mode: ${response.mayaMetadata.archetypeMode}`);
        console.log(
          `   📊 Wisdom Vector: ${response.mayaMetadata.wisdomVector}`,
        );

        if (response.voiceMetadata) {
          console.log(
            `   🔊 Voice Generated: ${response.voiceMetadata.audioUrl}`,
          );
          console.log(
            `   🎭 Voice Personality: ${response.voiceMetadata.personality}`,
          );
          console.log(
            `   ⚡ Energy Signature: ${response.voiceMetadata.energySignature}`,
          );
        } else {
    ...
    console.log(
      `🎯 Enhanced Hierarchy Tests: ${hierarchyTests}/${testQueries.length} successful\n`,
    );
...
async function testVoicePreviews() {
  try {
    ...
    console.log(
      `🎯 Voice Preview Tests: ${previewTests}/${archetypes.length} successful\n`,
    );
    return previewTests;
  } catch (error) {
...
async function runAllTests() {
  try {
    ...
    console.log(
      `   Individual Voices: ${voiceResults.successfulVoices}/${voiceResults.totalTests}`,
    );
    ...
    if (totalSuccessful >= totalTests * 0.8) {
      ...
      console.log(
        "🎭 Your consciousness system now SPEAKS with archetypal voices!",
      );
      ...
      console.log(
        "🔧 Some voice features may need ElevenLabs API key configuration",
      );
    } else {
      console.log("❌ Integration needs attention");
      console.log(
        "💡 Check ElevenLabs API configuration and network connectivity",
      );
    }
  } catch (error) {
