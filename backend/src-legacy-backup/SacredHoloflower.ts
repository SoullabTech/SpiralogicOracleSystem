...
export class SacredHoloflower {
  ...
  private calculateTransformationVectors(): void {
    this.state.activeTransformations.forEach((transformation) => {
      ...
      if (fromHouse && toHouse) {
        const dx =
          Math.cos(toHouse.angle) * toHouse.radius -
          Math.cos(fromHouse.angle) * fromHouse.radius;
        const dy =
          Math.sin(toHouse.angle) * toHouse.radius -
          Math.sin(fromHouse.angle) * fromHouse.radius;
  ...
  private recalculateBalance(): void {
    ...
    const totalIntensity = Array.from(elementBalance.values()).reduce(
      (sum, val) => sum + val,
      0,
    );
    ...
    const elementVariance =
      Array.from(elementBalance.values())
        .map((val) => Math.pow(val - avgIntensity * 3, 2))
        .reduce((sum, val) => sum + val, 0) / 4;

    this.state.overallBalance =
      1 - Math.sqrt(elementVariance) / (avgIntensity * 3 || 1);
    this.state.centerIntegration =
      Math.min(...Array.from(elementBalance.values())) /
      (avgIntensity * 3 || 1);
  }

  public integrateAether(): void {
    const aetherInfluence =
      this.state.centerIntegration * this.geometricRatios.phi;
  ...
  public getHouseByPosition(
    x: number,
    y: number,
    centerX: number,
    centerY: number,
  ): HoloflowerHouse | null {
    const dx = x - centerX;
    const dy = y - centerY;
    ...
    return (
      this.state.houses.find((house) => {
        const angleDiff = Math.abs(angle - house.angle);
        const normalizedDiff = Math.min(angleDiff, 2 * Math.PI - angleDiff);

        return (
          normalizedDiff < Math.PI / 12 &&
