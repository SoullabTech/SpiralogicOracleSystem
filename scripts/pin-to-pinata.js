...
async function pinDirectoryToPinata() {
  ...
  try {
    ...
    console.log("\n🌐 Access URLs:");
    console.log(
      `  - Pinata Gateway: https://gateway.pinata.cloud/ipfs/${response.data.IpfsHash}`,
    );
    console.log(
      `  - Cloudflare: https://cloudflare-ipfs.com/ipfs/${response.data.IpfsHash}`,
    );
