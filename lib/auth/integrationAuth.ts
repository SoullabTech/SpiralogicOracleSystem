...
export class IntegrationAuthService {
  ...
  private async createUserProfile(userId: string, onboardingData: OnboardingData) {
    const { data, error } = await this.supabase
      ...
      .insert({
        ...
        research_participation_consent: onboardingData.privacySettings.researchParticipation
      });
  ...
  async checkIntegrationReadiness(userId: string): Promise<{
    ...
    if (bypassingDetections && bypassingDetections.length > 0) {
      ...
      recommendations.push('Address current bypassing patterns before continuing');
    }
    ...
    if (activeGaps && activeGaps.length > 0) {
      ...
      recommendations.push('Complete current reflection gaps before accessing new content');
    }
  ...
  async exportUserData(userId: string) {
    try {
      const [
        ...
        this.supabase.from('integration_journeys').select('*').eq('user_id', userId),
        this.supabase.from('embodied_wisdom').select('*').eq('user_id', userId),
        this.supabase.from('bypassing_detections').select('*').eq('user_id', userId),
        this.supabase.from('community_interactions').select('*').eq('user_id', userId),
        this.supabase.from('content_interactions').select('*').eq('user_id', userId)
      ]);
