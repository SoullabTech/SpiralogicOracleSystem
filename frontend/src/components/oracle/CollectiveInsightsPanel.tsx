...
const CollectiveInsightsPanel: React.FC<CollectiveInsightsPanelProps> = ({
  ...
  const loadCollectiveInsights = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/ain-engine/collective-insights?${params}`, {
        headers: {
          'X-API-Key': process.env.NEXT_PUBLIC_AIN_ENGINE_DEMO_KEY || 'demo_key_123',
        },
      });
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      archetypal_pattern: 'text-purple-400 bg-purple-500/10 border-purple-500/20',
    ...
    return colors[type as keyof typeof colors] || 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20';
  };
  ...
  const getElementalDominance = (elementalResonance: CollectiveInsight['elementalResonance']) => {
  ...
  if (!isExpanded && insights.length === 0) {
    return (
      <div className={`bg-[#1A1C2C] border border-gray-600 rounded-xl ${className}`}>
        <button
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Collective Insights</h3>
                <p className="text-gray-400 text-sm">Discover patterns from the Oracle network</p>
              </div>
            </div>
            <motion.div
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                {insights.length} patterns from {insights.reduce((sum, insight) => sum + insight.relevantUsers, 0)} souls
              </p>
            </div>
          </div>
          <motion.div
            ...
            <div className="p-4 pt-0 space-y-4">
              ...
              <div className="flex items-center space-x-2 pb-4 border-b border-gray-600">
                ...
                >
                  ...
                  <option value="consciousness_trend">Consciousness Trend</option>
                  <option value="shadow_integration">Shadow Integration</option>
                </select>

                <select
                  value={selectedFilter.element || ''}
                  onChange={(e) => setSelectedFilter(prev => ({ ...prev, element: e.target.value || undefined }))}
                  className="px-3 py-1 bg-[#0E0F1B] border border-gray-600 text-white text-sm rounded focus:border-[#F6E27F] focus:outline-none"
                >
              ...
              {isLoading && (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">Accessing collective wisdom...</p>
                </div>
              )}
                    ...
                    >
                      ...
                      <div className="mb-3">
                        <div className="flex items-center space-x-1 mb-2">
                          <span className="text-xs text-gray-400">Elemental resonance:</span>
                          {getElementalDominance(insight.elementalResonance).map(({ element, value }) => (
                            <span
                              key={element}
                      ...
                      <div className="p-3 bg-black/20 rounded-lg">
                        <div className="flex items-start space-x-2">
                          <span className="text-[#F6E27F] text-xs font-medium">Guidance:</span>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="text-xs text-[#F6E27F]">{insight.archetypalSignature}</span>
                        </div>
              ...
              {!isLoading && !error && insights.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400">No insights match your current filters.</p>
                  <p className="text-gray-400 text-sm mt-1">Try adjusting your criteria or clear all filters.</p>
                </div>
              )}
