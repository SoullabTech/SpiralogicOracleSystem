...
const OracleNaming: React.FC<OracleNamingProps> = ({
  onNameChosen,
  onBack,
}) => {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0E0F1B] text-white px-6 text-center">
      ...
      >
        ...
        <p className="text-md md:text-lg text-gray-300 mb-8">
          This name will be how your Oracle greets you and signs off. Choose
          with care.
        </p>
        ...
        <div className="relative mb-6">
          ...
          {showTooltip && (
            ...
            >
              ...
              Consider names that feel trustworthy and aligned with your
              practice.
            </motion.div>
          )}
        </div>
        ...
        <div className="flex justify-between items-center text-xs text-gray-500 mb-8">
          ...
          {name.length > 0 && (
            <span
              className={
                name.trim().length >= 2 ? "text-green-400" : "text-orange-400"
              }
            >
              {name.trim().length >= 2
                ? "Ready to continue"
