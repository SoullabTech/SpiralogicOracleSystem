...
const HoloflowerDashboard: React.FC = () => {
  ...
  const handleSubmitCheckIn = async () => {
    ...
    try {
      const entryData = {
        user_id: user.id,
        fire:
          elementalStates.find((s) => s.element === "fire")?.intensity || 50,
        water:
          elementalStates.find((s) => s.element === "water")?.intensity || 50,
        earth:
          elementalStates.find((s) => s.element === "earth")?.intensity || 50,
        air: elementalStates.find((s) => s.element === "air")?.intensity || 50,
        aether:
          elementalStates.find((s) => s.element === "aether")?.intensity || 50,
        shadow:
          elementalStates.find((s) => s.element === "shadow")?.intensity || 50,
  ...
  const calculatePetalPath = (state: ElementalState): string => {
    ...
    const minRadius = 20;
    const radius =
      minRadius + (maxRadius - minRadius) * (state.intensity / 100);
  ...
  const getCoherenceLevel = (): number => {
    const average =
      elementalStates.reduce((sum, state) => sum + state.intensity, 0) /
      elementalStates.length;
    const variance =
      elementalStates.reduce(
        (sum, state) => sum + Math.pow(state.intensity - average, 2),
        0,
      ) / elementalStates.length;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto leading-relaxed">
            Attune to your elemental landscape. Adjust each petal to reflect
            your current state.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {selectedElement ? (
              ...
              >
                {(() => {
                  const state = elementalStates.find(
                    (s) => s.element === selectedElement,
                  );
              ...
              <div className="bg-[#1A1C2C] border border-gray-600 rounded-xl p-6 text-center">
                <p className="text-gray-400">
                  Click on an element petal to adjust its intensity
                </p>
              </div>
            )}
            ...
            <div className="bg-[#1A1C2C] border border-gray-600 rounded-xl p-6">
              <h3 className="text-lg text-[#F6E27F] font-medium mb-3">
                Brief Reflection
              </h3>
              <textarea
                value={reflection}
