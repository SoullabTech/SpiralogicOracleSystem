...
const voiceOptions = [
  ...
  { id: "aether-guide", label: "Aether Guide", description: "Transcendent & Universal" },
];

const personaOptions = [
  { id: "warm", label: "Warm", description: "Nurturing and supportive approach" },
  { id: "formal", label: "Professional", description: "Clear and structured guidance" },
  { id: "playful", label: "Playful", description: "Light-hearted with gentle wisdom" },
];
...
const interactionStyles = [
  ...
  { id: "comprehensive", label: "Comprehensive", description: "Deep exploration with context" },
];

const elementalPreferences = [
  { id: "fire", label: "Fire", color: "text-red-400", description: "Action, passion, creativity" },
  { id: "water", label: "Water", color: "text-blue-400", description: "Emotion, intuition, flow" },
  { id: "earth", label: "Earth", color: "text-green-400", description: "Grounding, stability, growth" },
  { id: "air", label: "Air", color: "text-cyan-400", description: "Ideas, communication, clarity" },
  { id: "aether", label: "Aether", color: "text-purple-400", description: "Spirit, transcendence, wisdom" },
];
    ...
    >
      <div className="bg-[#1A1C2C] border border-gray-600 rounded-xl p-6">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-bold text-[#F6E27F]">Personal Oracle Settings</h2>
            <p className="text-gray-400 mt-1">Customize your Oracle's personality and interaction style</p>
          </div>
        ...
        {!isEditing ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-semibold text-white mb-4">Oracle Identity</h3>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-white">
                      {voiceOptions.find(v => v.id === settings.voice)?.label || "Not selected"}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-white">
                      {personaOptions.find(p => p.id === settings.persona)?.label || "Not selected"}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Interaction Style</label>
                    <div className="text-white">
                      {interactionStyles.find(s => s.id === settings.interactionStyle)?.label || "Not selected"}
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Preferred Elements</label>
                    <div className="flex flex-wrap gap-2">
                      {settings.preferredElements?.map(elementId => {
                        const element = elementalPreferences.find(e => e.id === elementId);
                        return element ? (
                          <span
                            key={elementId}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {personaOptions.map((persona) => (
                        <button
                          key={persona.id}
                          onClick={() => setFormData({ ...formData, persona: persona.id as any })}
                          className={`w-full text-left px-4 py-3 border rounded-lg transition-colors ${
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {interactionStyles.map((style) => (
                        <button
                          key={style.id}
                          onClick={() => setFormData({ ...formData, interactionStyle: style.id as any })}
                          className={`w-full text-left px-4 py-3 border rounded-lg transition-colors ${
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Preferred Elements (Optional)</label>
                    <p className="text-xs text-gray-500 mb-3">Select elements you'd like your Oracle to emphasize</p>
                    <div className="grid grid-cols-1 gap-2">
                      {elementalPreferences.map((element) => (
                        ...
                        >
                          <div className={`font-medium ${formData.preferredElements?.includes(element.id) ? '' : element.color}`}>
                            {element.label}
                          </div>
