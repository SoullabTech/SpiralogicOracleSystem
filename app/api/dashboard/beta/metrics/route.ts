...
export async function GET(request: NextRequest) {
  try {
    ...
    const successfulVoiceEvents = voiceEvents.filter(e => e.success).length;
    ...
    const avgVoiceLatency = voiceEvents.length > 0 
      ? voiceEvents.reduce((sum, e) => sum + (e.latency_ms || 0), 0) / voiceEvents.length
      : 180;

    const sttEvents = voiceEvents.filter(e => e.event_type === 'stt');
    const ttsEvents = voiceEvents.filter(e => e.event_type === 'tts');
    
    const sttAccuracy = sttEvents.length > 0 
      ? (sttEvents.filter(e => e.success).length / sttEvents.length) * 100 
      : 93;
      
    const ttsSuccessRate = ttsEvents.length > 0
      ? (ttsEvents.filter(e => e.success).length / ttsEvents.length) * 100
      : 97;

    // Calculate Memory Performance Metrics
    const memorySuccessSessions = sessions.filter(s => 
    ...
    const feelsAliveFeedback = feedback.filter(f => 
    ...
    const onboardingFunnel = {
      torus_activated: onboardingEvents.filter(e => e.metadata?.onboardingMilestone === 'torus_activated').length,
      voice_flow_complete: onboardingEvents.filter(e => e.metadata?.onboardingMilestone === 'voice_flow_complete').length,
      memory_recall_success: onboardingEvents.filter(e => e.metadata?.onboardingMilestone === 'memory_recall_success').length,
      multimodal_analyzed: onboardingEvents.filter(e => e.metadata?.onboardingMilestone === 'multimodal_analyzed').length
    };
    ...
    const betaMetrics = {
      ...
      voiceHealth: {
        ...
        recentEvents: voiceEvents.slice(0, 10).map(event => ({
      ...
      userSatisfaction: {
        ...
        recentFeedback: feedback.slice(0, 10).map(item => ({
