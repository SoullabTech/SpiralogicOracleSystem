      ...
      if (lastStableCommit) {
        ...
        exec(`git reset --hard ${lastStableCommit}`, async (resetErr) => {
          if (resetErr) {
            ...
            await notify(
              "SLACK_WEBHOOK_URL",
              `Rollback failed: ${resetErr.message}`,
            );
            ...
            await notify(
              "SLACK_WEBHOOK_URL",
              "Successfully rolled back to stable state. System protected.",
            );
            await notify(
              "TELEGRAM",
              "Successfully rolled back to stable state. System protected.",
        ...
        exec("git reset --hard HEAD~1", async (resetErr) => {
          if (resetErr) {
            ...
            await notify(
              "SLACK_WEBHOOK_URL",
              `Emergency rollback failed: ${resetErr.message}. Manual intervention required.`,
            );
            await notify(
              "TELEGRAM",
              `Emergency rollback failed: ${resetErr.message}. Manual intervention required.`,
            ...
            await notify(
              "SLACK_WEBHOOK_URL",
              "Emergency rollback completed. Manual review recommended.",
            );
            await notify(
              "TELEGRAM",
              "Emergency rollback completed. Manual review recommended.",
