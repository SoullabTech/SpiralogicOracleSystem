...
async function attemptRepair() {
  ...
  await notify(
    "SLACK_WEBHOOK_URL",
    "Build failed. Attempting automated repair...",
  );
    ...
    if (success) {
      ...
      return new Promise((resolve) => {
        exec("npm run build && npm test", async (err) => {
          if (err) {
            console.error(
              "❌ Repair attempt failed, continuing to next strategy...",
            );
            resolve(false);
          } else {
            ...
            await notify(
              "SLACK_WEBHOOK_URL",
              "Automated repair successful! Build recovered.",
            );
            await notify(
              "TELEGRAM",
              "Automated repair successful! Build recovered.",
  ...
  await notify(
    "SLACK_WEBHOOK_URL",
    "All repair attempts failed. Triggering rollback...",
  );
  await notify(
    "TELEGRAM",
    "All repair attempts failed. Triggering rollback...",
